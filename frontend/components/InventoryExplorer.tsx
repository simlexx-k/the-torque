  ...
  return (
    <main className="product-page inventory-page">
      <section className="page-hero compact-page-hero">
        <div>
          <div className="page-kicker"><span>02</span> INDEXED INVENTORY</div>
          <h1>Find the machine<br/><em>behind the listing.</em></h1>
          <p>Search every indexed vehicle, narrow the signal by status and vehicle profile, then save or compare what matters.</p>
        </div>
        <div className="page-hero-stat">
          <Sparkles size={18} />
          <span><small>VISIBLE SIGNALS</small><strong>{filtered.length.toString().padStart(2, "0")}</strong></span>
        </div>
      </section>

      <section className="inventory-workbench">
        <div className="workbench-title-row">
          <div><SlidersHorizontal size={18} /><span>Discovery controls</span></div>
          <small>{activeFilters ? `${activeFilters} active filters` : "Full inventory"}</small>
        </div>

        <div className="inventory-controls-grid">
          <label className="product-search-field">
            <Search size={18} />
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search make, model, variant, location…" />
            {query && <button type="button" onClick={() => setQuery("")} aria-label="Clear search"><X size={15} /></button>}
          </label>
          <label className="product-select"><Filter size={16} /><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">All status</option><option value="available">Available</option><option value="reserved">Reserved</option><option value="sold">Sold</option><option value="price_drop">Price drop</option></select></label>
          <label className="product-select"><select value={make} onChange={(event) => setMake(event.target.value)}><option value="all">All makes</option>{makes.map((item) => <option key={item}>{item}</option>)}</select></label>
          <label className="product-select"><select value={body} onChange={(event) => setBody(event.target.value)}><option value="all">All body types</option>{bodies.map((item) => <option key={item}>{item}</option>)}</select></label>
          <label className="product-select"><ListFilter size={16} /><select value={sort} onChange={(event) => setSort(event.target.value)}><option value="latest">Latest signal</option><option value="price-high">Price: high to low</option><option value="price-low">Price: low to high</option><option value="year">Newest year</option><option value="mileage">Lowest mileage</option></select></label>
      ...
      <section className="inventory-results-section">
        <div className="results-heading-row">
          ...
          <small>{listingsQuery.isFetching ? "Refreshing source intelligence…" : `${listings.length} total records indexed`}</small>
        </div>
        ...
        {listingsQuery.isPending ? (
          ...
          <div className="product-state"><small>DATA LINK</small><h2>Inventory feed unavailable.</h2><p>{listingsQuery.error instanceof Error ? listingsQuery.error.message : "Unable to load inventory."}</p></div>
        ) : filtered.length ? (
          ...
          <div className="product-state"><small>FILTER RESULT</small><h2>No vehicle matches this signal.</h2><p>Reset the filters or broaden the search query.</p><button type="button" className="product-primary-button" onClick={reset}>Reset filters</button></div>
        )}
      </section>
    </main>
  );
